...
func TestPSQLScopeAndNativeParsing(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{[]string{"-c", "select 1 where 1=1"}, true, false},
		{[]string{"-d", "analytics"}, false, false},
		{[]string{"-danalytics"}, false, false},
		{[]string{"--dbname=analytics"}, false, false},
		{[]string{"analytics", "-Atc", "select 1"}, false, false},
		{[]string{"postgresql://app@localhost:5432/app_dev"}, false, true},
		{[]string{"-w", "postgres://app@localhost/app_dev", "-c", "select 1"}, false, true},
		{[]string{"host=localhost dbname=app_dev user=app"}, false, true},
		{[]string{"--", "host=localhost"}, false, true},
		{[]string{"-d", "postgres://app@localhost/app_dev"}, false, true},
		{[]string{"-dpostgres://db/analytics"}, false, true},
		{[]string{"-Ad", "postgresql://db/analytics"}, false, true},
		{[]string{"-d", "host=localhost dbname=app_dev"}, false, true},
		{[]string{"-dhost=localhost"}, false, true},
		{[]string{"--dbname", "postgresql://db/analytics"}, false, true},
		{[]string{"--dbname=postgres://db/analytics"}, false, true},
		{[]string{"--dbname=dbname=analytics"}, false, true},
		{[]string{"-d", "analytics", "-d", "host=other"}, false, true},
...
func TestSchemaRejectsInvalidDefinitions(t *testing.T) {
	for _, data := range []string{
		...
		customYAML + "passthrough: {flagValues: [{flag: \"--endpoint\"}]}\n",
		customYAML + "passthrough: {flagValues: [{flag: \"--endpoint\", contains: [\"\"]}]}\n",
		customYAML + "passthrough: {flagValues: [{flag: \"-k\", contains: [\"=\"]}]}\n",
		customYAML + "passthrough: {positionals: [{index: 0, prefixes: [\"a://\"]}, {index: 0, contains: [\"=\"]}]}\n",
		customYAML + "---\nversion: 1\n",
